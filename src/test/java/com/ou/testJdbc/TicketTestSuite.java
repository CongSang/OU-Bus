...
public class TicketTestSuite {
    ...
    public void testCreateTicketForNewTrip() {
        try {
            int expected = 1;
...
//        try {
//            int actual = t.createTicketFree(tripId); // Chuyen cac ve thanh FREE
//            Assertions.assertEquals(expected, actual
//                    , String.format("Dang ktra tripId = %d", tripId));
//        } catch (SQLException ex) {
//            Logger.getLogger(BusTestSuite.class.getName()).log(Level.SEVERE, null, ex);
...
//        try {
//            int actual = t.createTicketWithDraw(tripId); // Chuyen cac ve thanh WITHDRAW
//            Assertions.assertEquals(expected, actual
//                    , String.format("Dang ktra tripId = %d", tripId));
//        } catch (SQLException ex) {
//            Logger.getLogger(BusTestSuite.class.getName()).log(Level.SEVERE, null, ex);
...
//    }
    ...
    public void testSetTicketBooked(int tripId, int seatId, int customerID
        ...
        try {
            int actual = t.createTicketBooked(tripId, seatId
                    , customerID, employeeId, dateBook); // Chuyen 1 ve thanh BOOKED
            Assertions.assertEquals(expected, actual
                    , String.format("Dang ktra tripId = %d", tripId));
        } catch (SQLException ex) {
            Logger.getLogger(BusTestSuite.class.getName()).log(Level.SEVERE, null, ex);
    ...
    @Test
    public void testSetTicketThrow() {
        Assertions.assertThrows(ParseException.class, () -> {
            t.setTicketBookedBefore30min();
            t.setTicketFreeBefore5min();
        });
    }
}
